...
parser = argparse.ArgumentParser(
    description="""Small program for splitting Hall thruster diffusion data into test and validation sets.
...
parser.add_argument(
    "--val-dir-small",
    type=Path,
    default=Path("data/val_small"),
    help="Directory to put small validation set",
)
parser.add_argument(
    "--val-dir-large",
    type=Path,
    default=Path("data/val_large"),
    help="Directory to put large validation set",
)
parser.add_argument(
    "--frac-small",
    type=float,
    default=0.002,
    help="Fraction of training data to put in small validation set",
)
parser.add_argument(
    "--frac-large",
    type=float,
    default=0.1,
    help="Fraction of training to put in large validation set",
)
parser.add_argument(
    "-o",
    "--output_train_dir",
    type=Path,
    help="Output directory for training data. If not specified, the script operates in-place.",
)
...
def make_split(train_dir, test_dir, num_to_split):
    ...
    shutil.copyfile(train_dir / "norm_perf.csv", test_dir / "norm_perf.csv")
    shutil.copyfile(train_dir / "norm_fourier.csv", test_dir / "norm_fourier.csv")
...
if __name__ == "__main__":
    ...
    num_train = len(train_files) - num_to_split_small - num_to_split_large

    print(f"Files in small test set: {num_to_split_small}")
    print(f"Files in large test set: {num_to_split_large}")
    print(f"Files in large test set: {num_train}")

    # Copy training data if requested
    if output_train_dir is not None:
        shutil.copytree(train_dir, output_train_dir)
        train_dir = output_train_dir
